...
const ASSETS = {
  NIFTY: { name: "NIFTY 50", spot: 23124.5, step: 50 },
  BANKNIFTY: { name: "BANKNIFTY", spot: 51345.2, step: 100 },
  SENSEX: { name: "SENSEX", spot: 76543.2, step: 100 },
};
...
function getUpcomingExpiries(assetKey: string): string[] {
  ...
  if (assetKey === "BANKNIFTY") {
    ...
    while (count < 4) {
      ...
      const lastTuesdayCompare = new Date(
        lastTuesday.getFullYear(),
        lastTuesday.getMonth(),
        lastTuesday.getDate(),
      );
      const istTodayCompare = new Date(
        istNow.getFullYear(),
        istNow.getMonth(),
        istNow.getDate(),
      );
      ...
      if (lastTuesdayCompare < istTodayCompare) {
        ...
        lastTuesdayCompare.getTime() === istTodayCompare.getTime() &&
        isPastExpiryTime
      ) {
        isPast = true;
      }
...
export default function OptionChain() {
  ...
  const [expiry, setExpiry] = useState(
    () => getUpcomingExpiries("BANKNIFTY")[0],
  );
  ...
  const chainData = useMemo(
    () => generateOptionChain(selectedAsset.spot, selectedAsset.step),
    [assetKey],
  );
    ...
    <div className="space-y-3 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          <h2 className="text-sm font-bold text-white">
            Option Flow Analysis - {selectedAsset.name}
          </h2>
          <p className="text-[10px] text-slate-500 mt-0.5">
            <span className="text-emerald-400">CE</span> vs{" "}
            <span className="text-red-400">PE</span> Open Interest
          </p>
        </div>

        <div className="flex items-center gap-2">
          <div className="bg-[#1a1c21] border border-slate-800 rounded px-2 py-1 flex items-center">
            <span className="text-[9px] text-slate-500 uppercase tracking-wider mr-1.5 font-bold">
              Spot:{" "}
            </span>
      ...
      <div className="bg-[#101114] border border-slate-800/60 rounded-lg px-3 py-1.5 flex flex-wrap items-center justify-between gap-2.5 text-[10px]">
        ...
        <div className="flex items-center gap-3 text-slate-500 font-mono text-[9px]">
          <span>
            Feed ID:{" "}
            <strong className="text-slate-300">NSE_OC_SYNC_2026</strong>
          </span>
          <span className="hidden sm:inline border-l border-slate-800 pl-3">
            Current Active Expiry:{" "}
            <strong className="text-indigo-400 font-bold">{expiry}</strong>
          </span>
        </div>
      </div>
      ...
      <Card className="border-slate-800 overflow-hidden text-[11px] bg-[#101114]">
        <div className="grid grid-cols-12 bg-slate-800/80 border-b border-slate-700 text-[10px] font-bold text-slate-400 uppercase tracking-wider text-center">
          ...
          <div className="col-span-12 md:col-span-5 grid grid-cols-5 py-2 border-b md:border-b-0 md:border-r border-slate-700">
            <div className="text-center col-span-5 pb-1 mb-1 border-b border-slate-700 text-emerald-400 tracking-widest">
              Calls (CE)
            </div>
          ...
          <div className="col-span-12 md:col-span-5 grid grid-cols-5 py-2 border-t md:border-t-0 md:border-l border-slate-700">
            <div className="text-center col-span-5 pb-1 mb-1 border-b border-slate-700 text-red-400 tracking-widest">
              Puts (PE)
            </div>
            <div className="px-1 text-white">LTP</div>
            <div className="px-1">IV</div>
        ...
        <div className="font-mono divide-y divide-slate-800/50">
          {chainData.map((row, idx) => {
            ...
            const isATM =
              Math.abs(row.strike - spotPrice) <= selectedAsset.step / 2;
            ...
            return (
              ...
              >
                ...
                <div
                  className={cn(
                    "col-span-12 md:col-span-5 grid grid-cols-5 md:border-r border-slate-800 py-0.5",
                    isITM_CE ? "bg-emerald-950/10" : "",
                  )}
                >
                  <div className="px-1 text-slate-300">
                    {(row.ce.oi / 100000).toFixed(2)}
                  </div>
                  <div
                    className={cn(
                      "px-1",
                      row.ce.oiChange > 0 ? "text-emerald-400" : "text-red-400",
                    )}
                  >
                    {row.ce.oiChange > 0 ? "+" : ""}
                  ...
                  <div className="px-1 font-bold text-emerald-400">
                ...
                <div className="col-span-12 md:col-span-2 flex flex-col items-center justify-center bg-slate-900/50 border-x border-slate-800 py-1 font-bold text-[11px] text-white">
                  <span
                    className={cn(isATM ? "text-yellow-500" : "text-white")}
                  >
                    {row.strike}{" "}
                    {isATM && (
                      <span className="font-sans text-[8px] tracking-widest">
                        (ATM)
                      </span>
                    )}
                  </span>
                ...
                <div
                  className={cn(
                    "col-span-12 md:col-span-5 grid grid-cols-5 md:border-l border-slate-800 py-0.5",
                    isITM_PE ? "bg-red-950/10" : "",
                  )}
                >
                  <div className="px-1 font-bold text-red-400">
                  ...
                  <div
                    className={cn(
                      "px-1",
                      row.pe.oiChange > 0 ? "text-emerald-400" : "text-red-400",
                    )}
                  >
                    {row.pe.oiChange > 0 ? "+" : ""}
